...
class Account{
    ...
    const result = await pool.query(`SELECT * FROM accounts WHERE id = '${this.id}'`)
        if (result.length > 0) {
            ...
            await pool.query(`INSERT INTO accounts (id, name, bodyweight, level, xp, creationDate, skiptotal, skipstreak, squat, bench, deadlift) VALUES (
                '${this.id}', '${this.name}', 0, 1, 0, '${new Date().toDateString()}', 0, 0, 0, 0, 0)`)
        }
    ...
    async writeStatsToDB(logInfo = false) {
        ...
        const total = await pool.query(`SELECT SUM(settotal) FROM lifts WHERE userID = '${this.id}';`);            
        this.totalweight = total[0]["SUM(settotal)"];
        
        const totalsets = await pool.query(`SELECT COUNT(*) FROM lifts WHERE userID = '${this.id}';`);
    ...
    async logSet(movement, weight, reps, date){
        ...
        await pool.query(`INSERT INTO lifts (userID, movement, weight, reps, date, settotal, dateval, time) VALUES (
            '${this.id}',
            '${movement}',
            ${weight},
            ${reps},
    ...
    async repeatSet(weight, reps, repeats){
        ...
        const sets = await pool.query(`SELECT * FROM lifts WHERE userID = '${this.id}' ORDER BY dateval DESC, setid DESC;`);
        if (sets.length > 0) {
    ...
    async logCardio(movement, time, date, note, distance) {
        ...
        await pool.query(`INSERT INTO cardio (userid, movement, cardiotime, date, time, note, distance) VALUES (
            '${this.id}',
            '${movement}',
            ${time},
            '${today}',
    ...
    async getBreakdown(date) {
        ...
        const avgs = await pool.query(scoreQry);
    ...
    async getCardio(date) {
        ...
        const results = await pool.query(`SELECT * FROM cardio WHERE userid = '${this.id}' AND date = '${today}';`);
    ...
    async undoSet(sets){
        ...
        const results = await pool.query(`SELECT * FROM lifts WHERE userID = '${this.id}' ORDER BY dateval DESC, setid DESC;`);
        if (results.length == 0) {
            return false;
        }
        ...
        for (let i = 0; i < removeSets; i++) {
            ...
            const delResult = await pool.query(`DELETE FROM lifts WHERE setid = ${set.setid};`)
    ...
    async getTotalDays(){
        const allDates = await pool.query(`SELECT DISTINCT date FROM lifts WHERE userID = '${this.id}' ORDER BY date;`);
